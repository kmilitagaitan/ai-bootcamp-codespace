...
Copyright c⃝2014 Allen B. Downey.
Green Tea Press
9 Washburn Ave
Needham MA 02492
...
tributed by Freesound users: iluppai, wcﬂ10, thirsk, docquesting, kleeb, landup,
...
electrical signal and produces sound.
Microphones and speakers are called
...
frames (because 5,513 is half of 11,025).
The time between frames, or
...
a given cutoﬀfrequency are attenuated (that is, reduced in magnitude)
by a factor.
...
time.
2. PI2 is a constant that stores 2π.
Multiplying by PI2 converts from
...
The only diﬀerence is evaluate.
As we saw before, ts is the sequence of
...
scales. The version on the right cuts oﬀthe fundamental to show the harmonics
more clearly.
...
and frequency of the harmonics. Their amplitude drops oﬀin proportion to
frequency squared. For example the frequency ratio of the ﬁrst two harmonics
(200 and 600 Hz) is 3, and the amplitude ratio is approximately 9.
The
...
harmonics drops oﬀmore slowly. Speciﬁcally, amplitude drops in proportion
...
written x + iy, where i is the imaginary unit, √−1. You can think of x
...
corresponding frequency exceeds some cutoﬀfrequency.
...
and sawtooth waves drop oﬀin proportion to 1/f; the harmonics of the trian-
gle wave drop oﬀlike 1/f 2. Can you ﬁnd a waveform that has even and odd
harmonics that drop oﬀlike 1/f 2?
...
0.5
1.0
0.4980.5000.5020.5040.5060.5080.510
Time (s)
0.8980.9000.9020.9040.9060.9080.910
...
that the signal spends equal time at each frequency in this range.
Based
...
The y-axis shows frequency from 0 to 700 Hz. I cut oﬀthe top part of the
...
resolution, n/r, is the inverse of frequency resolution, r/n.
So if one gets
...
This tradeoﬀis called the Gabor limit and it is a fundamental limitation of
...
evaluate.
Make a wave that simulates a trombone glissando from C3 up
...
most likely, with probability that drops oﬀaccording to the Gaussian or
“bell” curve.
...
(left), it doesn’t look like much.
Nearly all of the power is at the lowest
...
tion function drops oﬀto zero quickly. For larger values, serial correlation is
stronger and drops oﬀmore slowly. With β = 1.7 serial correlation is strong
56
Chapter 5.
Autocorrelation
0
...
the estimated pitch might be oﬀby 50 Hz; in musical terms, the range from
...
correspond to frequencies 432 and 441 Hz.
The frequency precision using
...
Does the autocorrelation function drop oﬀquickly? Is there evidence of peri-
odic behavior?
...
But in this form it doesn’t help
...
compute M = cos(2πt ⊗f). Then we want to ﬁnd a so that y = Ma; in other
words, we want to solve a linear system.
NumPy provides linalg.solve,
...
M −1. That implies that M TM = I, which means we can check whether a
matrix is orthogonal by computing M TM.
...
close to 2I, which means M is almost orthogonal; it’s just oﬀby a factor of 2.
...
>>> wave.ys
[ 0.324+0.505j -0.505+0.324j -0.324-0.505j
0.505-0.324j]
...
we have:
A exp iφ0 · exp i2πft = A exp i(2πft + φ0)
...
is that dft doesn’t know the sampling rate.
The DFT assumes that the
...
dominant frequency is at 500 Hz, with harmonics dropping oﬀlike 1/f.
...
1. In that case, DFT(y) = y. Or if the length of y is suﬃciently small, you
...
17, 2012 to December 8, 2015.
The gray line is the raw data, the darker
...
square wave drop oﬀslowly, so it contains many high-frequency components.
90
Chapter 8.
Filtering and Convolution
0
...
leading edge of the window reaches the ﬁrst transition, and levels oﬀwhen
...
m=0
f[m]g[n + m]
where f is a wave array with length N, g is the window, and ⋆is the symbol
...
The diﬀerence is in the index of g:
...
and drops oﬀat a cutoﬀfrequency near 4000 Hz. But there is another feature
...
but it is not a very good one. The DFT drops oﬀsteeply at ﬁrst, but then
...
contains high-frequency harmonics that drop oﬀproportionally to 1/f, which
is relatively slow.
96
Chapter 8.
Filtering and Convolution
...
we saw with the boxcar window. So it does a better job of cutting oﬀthe
higher frequencies.
The reason it does so well is that the DFT of a Gaussian curve is also a
Gaussian curve. So the ratio drops oﬀin proportion to exp(−f 2), which is
...
where IDFT is the inverse DFT. A simple implementation of convolution takes
...
of the window.
If we slice oﬀthe bogus values, the result agrees with
...
and
fft correlate.
this signal. However, the autocorrelation function drops oﬀslowly, suggesting
...
of the Gaussian window to see what eﬀect they have on the cutoﬀfrequency.
...
sian curve is also a Gaussian curve.
...
Figure 9.2 shows the resulting wave and its spectrum.
The daily changes
...
Amplitude ratio
integral
Figure 9.3: Filters corresponding to the diﬀand diﬀerentiate operators (left)
...
then replaced the hs with 2πif.
Figure 9.3 (left) shows this ﬁlter; it is a
straight line.
...
corresponds to a simple ﬁlter in the frequency domain: it multiplies each
9.4.
Integration
107
0
...
9.5
Cumulative sum
In the same way that the diﬀoperator approximates diﬀerentiation, the cumu-
...
tooth, the amplitudes of the components drop oﬀmore quickly. In Chapter 2,
we saw that the components of the sawtooth drop oﬀin proportion to 1/f.
...
highest frequencies, where it drops oﬀa little faster.
110
Chapter 9.
Diﬀerentiation and Integration
0
...
differentiate on a signal.
Create a triangle wave and plot it.
Apply
...
gunshot.
Figure 10.2 (left) shows the waveform of the gunshot.
Next we
...
are substantially diﬀerent, and the diﬀerences are clearly audible.
Load
...
10.5
Proof of the Convolution Theorem
Well, I’ve put it oﬀlong enough. It’s time to prove the Convolution Theorem
(CT), which states:
DFT(f ∗g) = DFT(f)DFT(g)
...
the eﬀect of multiplying ek by G[k]. In mathematical terms, each ek is an
...
As a result, you might notice that the output
...
signal.
imp_sig = thinkdsp.Impulses([0.005, 0.3, 0.6,
0.9],
amps=[1,
0.5, 0.25, 0.1])
...
applications.
At the transmitter, the signal (which might contain speech,
11.2.
Amplitude modulation
129
0
...
1000
1200
Figure 11.2: Demonstration of amplitude modulation.
The top row is the
...
Figure 11.4 shows two examples.
The top row is the impulse train in the
...
the result after applying a low-pass ﬁlter with a cutoﬀat the folding frequency,
5512 Hz.
...
frequencies above the cutoﬀare removed completely, as if they hit a brick
wall.
