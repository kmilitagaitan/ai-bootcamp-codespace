...
keep track of the ﬁles that make up a project.
A collection of ﬁles under
...
 Andy Engle knows the diﬀerence between GB and GiB.
...
time.
The compiler chooses a location for each variable and records these
locations as part of the compiled program.1
...
turn oﬀoptimization while you are developing new code. Once the program
...
 Abstraction: An abstraction is a simpliﬁed representation of something
...
emacs.
Every once in a while I switch to a terminal window, which is a
...
And those are just the processes I am aware of.
...
1,073,741,823 in decimal, or 0x3ﬀf ﬀﬀin hexadecimal (the preﬁx 0x indicates
...
the virtual address space runs from 0 to 0xﬀﬀﬀﬀ. The size of this address
...
slow and take too much space.
So actual implementations are a bit more
complicated.
3.6.
Address translation
21
...
 The size of the oﬀset is determined by the page size. In this example the
page size is 210 B, so it takes 10 bits to specify a byte on a page.
 If a VA is 32 bits and the oﬀset is 10 bits, the remaining 22 bits make
...
save its state, and then run another process.
...
Abstractly:
 A “ﬁle system” is a mapping from each ﬁle’s name to its contents. If
...
 Buﬀering: As I mentioned, when you write a ﬁle, the operating system
...
Chapter 5.
More bits and bytes
For example, the integer representation of 1310 is b1101. In ﬂoating point,
...
protocols.
ASCII code 7 is a bell; on some terminals, printing it makes a
sound.
5.5.
Representing strings
37
...
of my name, interpreted as a double-precision ﬂoating point number, is
...
 malloc, which takes an integer size, in bytes, and returns a pointer to
...
 free, which takes a pointer to a previously allocated chunk and deallo-
...
Often there is a trade-oﬀbetween safe memory management and performance.
...
For some programs, memory leaks are ok.
...
 Eventually, a process might ﬁll its virtual address space (or the usable
...
Chapter 7.
Caching
 A status register, or ﬂag register, that contains information about the
...
 Jump/branch: Changes the program counter, causing the ﬂow of execu-
...
one big one.
The trade-oﬀbetween speed, size, and cost is the fundamental reason for
...
the interrupted process to resume.
...
 Processes might be limited by diﬀerent resources. A process that does
...
Pthreads for short.
The POSIX standard deﬁnes a thread model and an
...
 Access to the queue is not thread safe. Diﬀerent threads could access
array, next_in, and next_out at the same time and leave the queue in
a broken, “inconsistent” state.
 If the consumer is scheduled ﬁrst, it ﬁnds the queue empty, print an error
